...
namespace caffe
{
  ...
  void Solver::Init()
  {
    ...
    { // P2PSync does other solvers if they exist
  ...
  void Solver::Step(int iters)
  {
    ...
    while (iter_ < stop_iter)
    {
      ...
      if (test_and_snapshot ||
          (!first_loop && param_.test_interval() && iter_ % param_.test_interval() == 0 && iterations_last_ >= 0))
      {
        iteration_timer_->Start();
      ...
      if (epoch_count > 0UL)
      {
        epochs = (double)(iters * param_.iter_size() * bps * Caffe::solver_count()) / epoch_count;
        epochs_passed = (double)(iter() * param_.iter_size() * bps * Caffe::solver_count()) / epoch_count;
      }
      ...
      if (this->param_display() && (display || rel_iter <= 2 || iter_ + 1 == stop_iter))
      {
        ...
        if (rel_iter > 2)
        { // we skip 0,1,2 for correct benchmarking
          total_lapse_ += lapse;
          ...
          LOG_IF(INFO, rank_ == 0) << "    " << net_->print_current_device() << " Iteration "
                                   ...
                                   << " iter/s, " << bps * per_s << " img/sec, " << lapse << "s/" << (iter_ - iterations_last_)
  ...
  bool Solver::Solve(const char *resume_file)
  {
    ...
    if (param_.snapshot_after_train() && (!param_.snapshot() || iter_ % param_.snapshot() != 0) && rank_ == 0)
    {
      Snapshot();
    }
  ...
  {
    std::string al(align, ' ');
    float perf_ratio = total_lapse() > 0. ? (relative_iter() > 2 ? relative_iter() - 2 : 0) / total_lapse() : 0.F;
